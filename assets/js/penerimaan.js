...
// Update status
function updateAdmissionStatus(newStatus) {
  if (!currentAdmissionId) return;

  const adm = admissions.find((a) => a.id === currentAdmissionId);
  if (adm) {
    adm.status = newStatus;
    adm.note = document.getElementById("admNote").value;

    // Re-render
    renderAdmissionTable();
    updateAdmissionStats();

    alert(`✅ Status diperbarui menjadi: ${formatAdmissionStatus(newStatus)}`);
    closeAdmissionModal();
  }
}
